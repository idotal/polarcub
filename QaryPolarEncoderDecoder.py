...
class QaryPolarEncoderDecoder:
    ...
    def encode(self, xVectorDistribution, information):
        ...
        Args:
            xVectorDistribution (VectorDistribution): in a memory-less setting, this is essentially a vector with a-priori entries for P(X=j) for all j in F_q
        ...
        encodedVector, next_uIndex, next_informationVectorIndex = self.recursiveEncodeDecode(information, 0,
                                                                                             0,
                                                                                             xVectorDistribution)
    ...
    def decode(self, xVectorDistribution, xyVectorDistribution):
        ...
        Args:
            xVectorDistribution (VectorDistribution): in a memory-less setting, this is essentially a vector with a-priori entries for P(X=j) for each j in F_q.
            xyVectorDistribution (VectorDistribution): in a memory-less setting, this is essentially a vector with a-posteriori entries for P(X=j) for each j in F_q. That is, entry i contains P(X=j,Y=y_i) for each j in F_q.
        Returns:
    ...
    def genieSingleDecodeSimulation(self, xVectorDistribution, xyVectorDistribution, trustXYProbs):
        ...
        decodedVector, next_uIndex, next_informationVectorIndex = self.recursiveEncodeDecode(information, 0,
                                                                                             0,
                                                                                             xVectorDistribution,
                                                                                             xyVectorDistribution,
                                                                                             marginalizedUProbs)
    ...
    def genieSingleEncodeSimulation(self, xVectorDistribution):
        ...
        Args:
            xVectorDistribution (VectorDistribution): in a memory-less setting, this is essentially a vector with a-priori entries for P(X=0) and P(X=1)
        Returns:
        ...
        for probTuple in marginalizedUProbs:
            ...
            exit(1)
            TVvec.append(abs(probTuple[0] - probTuple[1]))  # This has to be changed
            Hvec.append(sum([QaryMemorylessDistribution.eta(prob) for prob in probTuple]))

        return encodedVector, TVvec, Hvec

    def recursiveEncodeDecode(self, information, uIndex, informationVectorIndex,
                              xVectorDistribution, xyVectorDistribution=None, marginalizedUProbs=None, listSize=1):
        ...
        Args:
            ...
            xVectorDistribution (VectorDistribution): in a memory-less setting, this is essentially a vector (whose length is a function of the recursion depth) with a-priori entries for P(X=j) for all j in F_q
            xyVectorDistribution (VectorDistribution): in a memory-less setting, this is essentially a vector (whose length is a function of the recursion depth) with a-posteriori entries for P(X=j) for all j in F_q. A None value means we are encoding.
        ...
        if len(xVectorDistribution) == 1:
            ...
            # TODO: should we return the marginalized probabilities?
            ...
            return encodedVector, uIndex + 1, next_informationVectorIndex
            ...
            plusEncodedVector, next_uIndex, next_informationVectorIndex = self.recursiveEncodeDecode(information,
                                                                                                       next_uIndex,
                                                                                                       next_informationVectorIndex,
                                                                                                       xPlusVectorDistribution,
                                                                                                       xyPlusVectorDistribution,
                                                                                                       marginalizedUProbs)

            halfLength = len(xVectorDistribution) // 2
            for half_i in range(halfLength):
                encodedVector[2 * half_i] = (minusEncodedVector[half_i] + plusEncodedVector[half_i]) % self.q
                encodedVector[2 * half_i + 1] = plusEncodedVector[half_i]
...
def genieEncodeDecodeSimulation(length, make_xVectorDistribution, make_codeword, simulateChannel,
    ...
    for trialNumber in range(numberOfTrials):
        genieSingleRunSeed = genieSingleRunSeedRNG.randint(1,
                                                           1000000)  # The seed used to randomly pick the common randomness for this genie run
