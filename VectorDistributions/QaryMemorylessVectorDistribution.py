...
class QaryMemorylessVectorDistribution(VectorDistribution.VectorDistribution):
    ...
    def __init__(self, q, length):
        ...
        assert (length > 0)
        self.probs = np.empty(
            (length, q), dtype=np.float)  # so, probs[i][x] equals the probability of x transmitted or received at time i
        self.probs[:] = np.nan
        self.length = length
    ...
    def calcMarginalizedProbabilities(self):  # Q: why are marginalized probs only calculated for input x = 0?
        assert (len(self) == 1)
